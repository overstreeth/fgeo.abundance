...
```{r}
plots <- map_sp(census, c("hybapr", "faraoc"))

# Adding a new layer to one element of the plots' list
plots[["hybapr"]] + geom_vline(aes(xintercept = 300), colour = "red")
```

And this is how you can apply the same new layer to all the plot-elements of the output list (note that the function `+` is applied to each item in the `plots` list):

```{r}
lapply(plots, `+`, geom_vline(aes(xintercept = 300), colour = "red"))
