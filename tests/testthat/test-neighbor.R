...
describe("neighbor_*() inputs", {
  it("fails with informative messages when fed with common bad-inputs", {
    no_df <- 1
    expect_error(
      count_neighbor(no_df, r = 20, plotdim = c(320, 500)),
      "is not TRUE"
    )
    expect_error(
      count_neighbor(tree, .subset = no_df, r = 20, plotdim = c(320, 500)),
      "is not TRUE"
    )
    bad_nm <- data.frame(bad_names = 1)
    expect_error(
      count_neighbor(bad_nm, r = 20, plotdim = c(320, 500)),
      "Invalid.*Ensure your data set has these variables"
    )
    expect_error(
      count_neighbor(tree, .subset = bad_nm, r = 20, plotdim = c(320, 500)),
      "Invalid.*Ensure your data set has these variables"
    )
    expect_error(
      count_neighbor(tree, plotdim = c(320, 500)),
      "argument.*is missing"
    )
  })
  ...
  it("warns if detects a stem (not tree) table", {
    ...
    expect_warning(count_neighbor(stem, r = 20), "Duplicated values")
  })
})
...
describe("neighbor_*() side effects", {
  it("outputs a suppressable message (not a `cat()` like notification)", {
    expect_silent(suppressMessages(count_neighbor(tree, r = 20)))
  })
})
...
describe("neighbor_*()", {
  ...
  pd <- c(320, 500)
  r <- 20
  it("count_neighbor() outputs equal via group_by() and split()", {
    dfs <- split(quads, quads$quadrat)
    out_split <- reduce(lapply(dfs, count_neighbor, r = r, plotdim = pd), rbind)

    byquad <- group_by(quads, quadrat)
    out_neigh <- count_neighbor(byquad, r = r, plotdim = pd)
