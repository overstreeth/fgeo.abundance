context("neighbor")

set.seed(1)
tree <- fgeo.data::luquillo_tree5_random[1:10, ]

describe("neighbor_count() and neighbor_basal_area()", {
  it("outputs the equivalent to ctfs::NeighborDensityes() counterpart", {
    skip_if_not_installed("ctfs")
    ctfs_n <- ctfs::NeighborDensities(tree, type = "count")
    ctfs_b <- ctfs::NeighborDensities(tree, type = "basal")
    expect_known_output(ctfs_n, "ref-NeighbourDensities_count", print = TRUE)
    expect_known_output(ctfs_b, "ref-NeighbourDensities_basal_area", print = TRUE)
    
    fgeo_n <- neighbor_count(tree)
    fgeo_b <- neighbor_basal_area(tree)
    expect_equal(fgeo, ctfs)
  })
})
