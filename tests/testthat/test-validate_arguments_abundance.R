...
test_that("throws ERROR with wrong input", {
  ...
  expect_error(
    validate_arguments_abundance(censdata = cns5_mini, mindbh = "wrong")
  )
  expect_error(
    validate_arguments_abundance(censdata = cns5_mini, dbhunit = "wrong")
  )
  wrong_length <- rep(1:3, nrow(cns5_mini))
  expect_error(
    validate_arguments_abundance(censdata = cns5_mini, split1 = wrong_length)
  )
  expect_error(
    validate_arguments_abundance(censdata = cns5_mini, split2 = wrong_length)
  )
})
...
test_that("is SILENT with good arguments", {
  ...
  expect_silent(
    validate_arguments_abundance(censdata = cns5_mini, split1 = cns5_mini$sp)
  )
  MeasureID <- rep(1, nrow(cns5_mini))
  expect_silent(
    validate_arguments_abundance(censdata = cns5_mini, split1 = MeasureID)
  )
  MeasureID <- rep(1, nrow(cns5_mini))
  expect_silent(
    validate_arguments_abundance(censdata = cns5_mini, split2 = MeasureID)
  )
})
