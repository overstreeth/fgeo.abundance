...
multipaged <- marrangeGrob(maps, nrow = 2, ncol = 2)
multipaged
```

Now you can save your multi-paged maps to a .pdf or other device:
...
# Supplement: Mapping as Part of a Pipe

Both `map_sp()` and  `map_sp_pdf()` can be used as part of a pipe. (With the pipe operator (`%>%`; Control/Command + Shift + M) you can chain multiple functions more expressively than with multiple function calls [http://style.tidyverse.org/pipes.html].)

```{r}
census %>% 
  dplyr::filter(sp == "hybapr") %>% 
  # Print map to screen and continue pipe
  forestr::map_sp(unique(.$sp)) %>% 
  # Line above invisibly retuns the filtered census, so it can be used next
  count(sp)

# Compare: Above the count is done after filtering.
census %>% count(sp)
```
